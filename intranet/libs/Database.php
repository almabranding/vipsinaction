...
class Database extends PDO {
    ...
    function DuplicateRow($table, $id_field, $id) {
        $row = $this->selectOne("SELECT * FROM " . $table . " WHERE " . $id_field . " = '" . $id . "' LIMIT 1");
        unset($data);
        foreach ($row as $key => $val) {
            if ($key != $id_field) {
                $data[$key]=($key == 'created_at' || $key == 'updated_at')?  Model::getTimeSQL():$val;
            }
        }
