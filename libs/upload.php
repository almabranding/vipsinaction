...
class upload extends Model {
    ...
    public function upload($sub = 'temp/', $name = 'pic',$result=true) {
        ...
        if (!is_dir(ROOT_UPLOAD))
            mkdir(ROOT_UPLOAD);
        $uploadDir = ROOT_UPLOAD . $sub . '/';
        if (!is_dir($uploadDir))
            mkdir($uploadDir);
    ...
    public function crop() {
        ...
        $filepath = ROOT_UPLOAD . $_POST['filefolder'] . '/';
        $rel = $_POST['rel'];
        $targ_w = $_POST['w'] * $rel;
        $targ_h = $_POST['h'] * $rel;
    ...
    public function insertImg($img) {
        ...
        $file_name = (file_exists(ROOT_UPLOAD . $rute . 'original/' . $img['file'])) ? $img['nameFile']. '_' . rand().'.'.$img['extension']: $img['file'];
        $data = array(
            'file_name' => $file_name,
        ...
        if (!is_dir(ROOT_UPLOAD . $rute . 'original'))
            mkdir(ROOT_UPLOAD . $rute . 'original/', 0777, true);
        copy(ROOT_UPLOAD . 'temp/' . $img['file'], ROOT_UPLOAD . $rute . 'original/' . $file_name);
        $thumb = new thumb();
        $thumb->loadImage(ROOT_UPLOAD . $rute . 'original/' . $file_name);
        $thumb->crop(534,350);
        $thumb->save(ROOT_UPLOAD . $rute . $file_name);
        $thumb->reset();
        $thumb->crop(251,163);
        $thumb->save(ROOT_UPLOAD . $rute . 'thumb_' . $file_name);
        $thumb->reset();
        $thumb->crop(250,250);
        $thumb->save(ROOT_UPLOAD . $rute . 'thumb_250x250_' . $file_name);
        unlink(ROOT_UPLOAD . 'temp/' . $img['file']);
        $data['id'] = $photo_id;
        return $data;
    }
